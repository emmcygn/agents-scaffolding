...
**Pre-flight checklist:** `plans/BOOTSTRAP.md` (run before first task and after session recovery)
...
Open two Claude Code sessions. Tell Terminal 1 "You are Agent A" and Terminal 2 "You are Agent B". Each reads `plans/ORCHESTRATOR.md` and runs autonomously through all tasks. They coordinate via STATE.md and HANDOFF.md on disk.
...
1. Runs the bootstrap task as a pairing session (both agents, same context)
2. Launches Agent A and Agent B tasks as parallel background agents
3. Waits for completion notifications, checks STATE.md
4. Resolves dependencies (if Agent B Task N needs Agent A Task M, waits for A to finish)
5. Continues launching the next task for each agent as they complete
6. Handles pairing tasks in the main context

### Agent A — {{AGENT_A_ROLE}}
**Owns:** {{list of modules/files Agent A is responsible for}}
**Plans:** `plans/agent-a/task-01.md` through `task-NN.md`

### Agent B — {{AGENT_B_ROLE}}
**Owns:** {{list of modules/files Agent B is responsible for}}
**Plans:** `plans/agent-b/task-01.md` through `task-NN.md`
...
- **Pairing tasks** — both agents work on shared files. Coordinate through the shared contract file (the interface boundary).
...
- **Agent B can work ahead** on configuration, documentation, UI scaffolding, and deployment at any time — these typically have no upstream dependency after the bootstrap task.
...
- **Session recovery:** If context is lost, re-read `plans/STATE.md` → find last completed task → resume from next. Read `plans/HANDOFF.md` to catch up on what the other agent delivered.
...
- The task plan files (`plans/agent-{a,b}/task-XX.md`) are the master plans — consult them before each task
...
1. **Plan First**: Write plan to `tasks/todo.md` with checkable items per task plan
